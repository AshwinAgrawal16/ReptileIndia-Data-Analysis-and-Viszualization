require(rinat)
library(bdvis)
library(randomForest)
library(raster)
library(bdvis)
library(maptools)
inat=get_inat_obs_project("reptileindia") 

##plotting the species on the india map.
## inat[,16:15]=longitude and lattitude
plot(inat[,16:15], cex=0.5, col='blue')
data(wrld_simpl)
plot(wrld_simpl, add=TRUE)

##World climate data
wc <- getData('worldclim', res=10, var='bio')

## Now extracting climate data for our species
bfc <- extract(wc, inat[,16:15])
## Bio1-bio19 are climatic parameters which influence the species survival.
head(bfc)



##For ecological niche species
i <- which(is.na(bfc[,1]))
print(i)


##Ploting the two important parameters.
plot(bfc[ ,'bio1'] / 10, bfc[, 'bio12'], xlab='Annual mean temperature (C)',
     ylab='Annual precipitation (mm)')



##Now as our data contains only 748 points so we apply Machine Learning algorithms
##we may get biased results,so we increase the data by dynamic resampling.


##Removal NA values for analysis.
inat1<-inat[-i,]

##Getting the range of the values
e <- extent(SpatialPoints(inat1[, 16:15]))
print(e)


## Now generating 1000 points by resampling
set.seed(0)
bg <- sampleRandom(wc, 1000, ext=e)
dim(bg)

##Setting the actual data value class as 1 and generated data class as 0
d <- rbind(cbind(pa=1, bfc), cbind(pa=0, bg))
d <- data.frame(d)
dim(d)


## Now our data sample is ready for application of algorithms

##Removing NA values for Random Forest Classification model
d<-na.exclude(d)

##Class to be predicted
fpa <- as.factor(d[, 'pa'])

##Tuning the model parameters
trf <- tuneRF(d[, 2:ncol(d)], fpa)
print(trf)

##Mtry of the model
mt <- trf[which.min(trf[,2]), 1]
print(mt)

##Random Forest Classification model
crf <- randomForest(d[, 2:ncol(d)], fpa, mtry=mt)
print(crf)


plot(crf)

##This gives the impotance given to different climatic parameters by random forest
##classifier for classification
importance(crf)
varImpPlot(crf)

##The number of tress used is default 500 and incfesaing the tress will only over
## the model because of less number of the data points.


##Now Random Forest Regression Model

##Tuning the model
trf <- tuneRF(d[, 2:ncol(d)], d[, 'pa'])
print(trf)

##mtry for the model
mt <- trf[which.min(trf[,2]), 1]
print(mt)

##Now regression models
rrf <- randomForest(d[, 2:ncol(d)], d[, 'pa'], mtry=mt)
print(rrf)

##Importance of the predictors
plot(rrf)

importance(rrf)
varImpPlot(rrf)





##Now we will predict where the reptile species will be prominent on the india map
##by using heat table

##Here e extent of the values calculated above
## wc=world climate data
rp <- predict(wc, rrf, ext=e)
plot(rp)

